<template>
  <AHeader>
    ...
    <template #subheader>
      <UButtonGroup size="2xs" class="h-10 mr-4" orientation="horizontal">
        <UButton icon="fluent:align-space-fit-vertical-20-filled" size="2xs" @click="setList">
          List
        </UButton>
        <UButton icon="fluent:grid-24-filled" size="2xs" @click="setGrid">
          Grid [{{ gridSize }}]
        </UButton>
      </UButtonGroup>
  ...
  <div v-if="viewMode == 'list'" class="grid grid-cols-1 gap-4 pt-16">
    <div ref="media-list" class="h-dvh flex flex-col justify-start items-stretch p-8 gap-4 overflow-y-auto pb-48">
      <ImageCard v-for="file in visibleFiles" :key="file.name" :mode="mode" :file="file" @analyze-image="analyzeImage"
        @make-square="attemptMakeSquare" @set-focus-tag="setFocusTag" @crop-file="cropTarget = file"
        @remove-bg="removeBackground" @delete-file="deleteFile(file)" />
    </div>
  </div>
  <div v-else class="pt-16">
    <div ref="media-list" class="h-dvh grid p-8 gap-4 overflow-y-auto pb-48" :class="[`grid-cols-${gridSize}`]">
      <ImagePoster v-for="file in visibleFiles" :key="file.name" :file="file"
        :active="loaders.hasActiveLoaders(Prefixes.ANALYZE + file.name)"
        :pending="loaders.hasQueuedLoaders(Prefixes.ANALYZE + file.name)" @expand="() => expandFile(file)" />
    </div>
  </div>
  ...
  < !--Bottom Rounded Toolbar-->
    <div class=" w-full fixed flex flex-col-reverse justify-center items-center bottom-6 gap-2 pointer-events-none">
      <!-- Navigation -->
      <div v-if="visibleFiles.length" class="flex justify-center gap-8 pointer-events-none">
        <div
          ...
          <UButton icon="fluent:arrow-previous-16-filled" variant="ghost" size="sm" color="white"
            class="rounded-full p-2" :style="{ opacity: fileStore.page === 1 ? 0.2 : 1 }"
            :disabled="fileStore.page === 1" @click="setPage(1)" />
          <UButton icon="fluent:chevron-left-16-filled" variant="ghost" size="sm" color="white" class="rounded-full p-2"
            :style="{ opacity: fileStore.page === 1 ? 0.2 : 1 }" :disabled="fileStore.page === 1"
            @click="setPage(fileStore.page - 1)" />
          <span class="text-md text-center">{{ fileStore.page }}/{{ fileStore.pages }}</span>
          <UButton icon="fluent:chevron-right-16-filled" variant="ghost" size="sm" color="white"
            class="rounded-full p-2" :style="{ opacity: fileStore.page === fileStore.pages ? 0.2 : 1 }"
            :disabled="fileStore.page === fileStore.pages" @click="setPage(fileStore.page + 1)" />
          <UButton icon="fluent:arrow-next-16-filled" variant="ghost" size="sm" color="white" class="rounded-full p-2"
            :style="{ opacity: fileStore.page === fileStore.pages ? 0.2 : 1 }"
            :disabled="fileStore.page === fileStore.pages" @click="setPage(fileStore.pages)" />
        </div>
      </div>

      <!-- Batch Tools -->
      <div v-if="directoryStore.workingDirectory !== ''" class="flex justify-center gap-12 pointer-events-none">
        <div class="p-2 bg-slate-950/90 rounded-full shadow-lg shadow-black/25 z-10 pointer-events-auto border-none"
          :class="[activityEffect]" :style="animationEffect">
          <div class="flex justify-center items-center gap-4">
            <UTooltip text="Upload Images" :popper="{ arrow: true, placement: 'top' }">
              <UButton icon="fluent:image-add-32-light" variant="solid" color="indigo" class="rounded-full p-2"
                :disabled="showActivity" size="lg" :ui="{ icon: { size: { lg: 'w-8 h-8' } } }" @click="uploadFiles" />
            </UTooltip>
            <UTooltip text="Make All Square" :popper="{ arrow: true, placement: 'top' }">
              <UButton icon="fluent:color-background-20-regular" variant="solid"
                :color="fileStore.hasNonSquareFiles ? 'emerald' : 'white'"
                :disabled="!fileStore.hasNonSquareFiles || showActivity" class="rounded-full p-2" size="lg"
                :ui="{ icon: { size: { lg: 'w-8 h-8' } } }" @click="makeAllSquare" />
            </UTooltip>
            <UTooltip text="Scan Directory" :popper="{ arrow: true, placement: 'top' }">
              <UButton icon="fluent:slide-multiple-search-20-regular" variant="solid" size="lg" class="rounded-full p-2"
                :disabled="loaders.hasActiveLoaders(Prefixes.ANALYZE) ||
                  loaders.hasQueuedLoaders(Prefixes.ANALYZE) ||
                  showActivity
                  " :ui="{ icon: { size: { lg: 'w-8 h-8' } } }" @click="analyzeDirectory" />
            </UTooltip>
          </div>
        </div>
      </div>
    ...
    <UNotifications />
    <div v-if="loaders.getMax(Prefixes.ANALYZE)" class="fixed bottom-4 right-12 w-1/4">
      <UNotification :id="1" :timeout="0" title="Analyzing Images">
        <template #description>
          <div class="flex flex-col gap-2">
            ...
            <UProgress :value="loaders.getMax(Prefixes.ANALYZE) - loaders.analyzeQueue"
    ...
    <UModals />
    <USlideovers />
    <CropModal :open="!!cropTarget" :image="cropTarget" :save-callback="resolveImageEdit" @close="closeCropModal" />
    <PreviewModal :open="!!removalTarget && !fileStore.acceptCutoutResults" :image="removalTarget"
      :preview="removalPreview" @save="resolveBgRemoval(true)" @discard="resolveBgRemoval(false)" />
    <TagPreviewer :open="!!focusTag" :tag="focusTag" @close="() => (focusTag = null)" />
    <ImageOverlay :file="expandedFile" @close="() => expandedFile = null" />
</template>
  ...
  import ImagePoster from "~/components/ui/ImagePoster.vue";
  ...
  const expandedFile = ref<ImageFile | null>(null);
  const expandFile = (file: ImageFile) => {
  ...
  const setGrid = () => {
    if (viewMode.value == 'list') {
      ...
      // step grid size through list
      const curIdx = sizes.findIndex((s) => s == gridSize.value)
      gridSize.value = sizes[(curIdx + 1) % 5]
    }
  }
